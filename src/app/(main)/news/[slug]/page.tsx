...
export default function NewsPage() {
  ...
  const parentLink =
    mapping.MENU["/news"][locale] &&
    data?.parent?.ID === Number(mapping.MENU["/news"][locale])
      ? ""
      : `/${data?.parent?.ID}`;
  ...
  const createPageLink = (page: number) =>
    router.push(
      `/news/${id}${locale === "eng" ? "?lang=eng&" : "?"}${page > 1 ? "page=" + page : ""}`,
    );
    ...
    <>
      ...
      <div className="py-6">
        ...
        {data?.post && data?.post.length > 0 ? (
          ...
          <PageDetail ID={data?.baiviet?.ID as string} type="news" />
        )}
      </div>
    </>
  );
