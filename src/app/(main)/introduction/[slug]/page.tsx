...
export default function IntroductionPage() {
  ...
  const parentLink =
    mapping.MENU["/introduction"][locale] &&
    data?.parent?.ID === Number(mapping.MENU["/introduction"][locale])
      ? ""
      : `/${data?.parent?.ID}`;
  ...
  const createPageLink = (page: number) =>
    router.push(
      `/introduction/${id}${locale === "eng" ? "?lang=eng&" : "?"}${page > 1 ? "page=" + page : ""}`,
    );
    ...
    <>
      ...
      <div className="py-6">
        ...
        {id === "20450" ? (
          ...
          <PageDetail ID={data?.baiviet?.ID as string} type="introduction" />
        )}
      </div>
    </>
  );
